...
import { fetchAdminHistory } from '../../../services/api/historyService'
...
function useAdminHistoryData(initialLimit = 20) {
    ...
    const loadHistory = useCallback(async () => {
        ...
        try {
            ...
            const normalized = items.map((item) => ({
                ...
                status: item.conversionStatus?.toLowerCase?.() ?? item.status?.toLowerCase?.() ?? 'pending',
                created: formatTimestamp(item.requestTimestamp ?? item.createdAt),
            ...
            setTotal(paginationData.total ?? normalized.length)
            setPages(paginationData.pages ?? 1)
        } catch (reason) {
            setError(reason?.response?.data?.message ?? reason.message ?? 'Unable to load administration history.')
            setRawRows([])
        } finally {
            setIsLoading(false)
        }
...
export default useAdminHistoryData
